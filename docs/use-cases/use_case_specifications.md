...
| Tester | UC1 Register Issue, UC2 Add Comment(extend/include로 참여), UC3 Search Issues, UC4 View Issue Detail, UC6 Change Issue State, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW/REOPENED 이슈) |
| Dev | UC1 Register Issue, UC2 Add Comment(extend/include로 참여), UC3 Search Issues, UC4 View Issue Detail, UC6 Change Issue State, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW/REOPENED 이슈) |
| PL | UC1 Register Issue, UC2 Add Comment(extend/include로 참여), UC3 Search Issues, UC4 View Issue Detail, UC5 Assign / Update Issue Assignment, UC6 Change Issue State, UC7 Manage Dependency, UC9 Manage Deleted Issue, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW/REOPENED 이슈), UC16 Change Priority |
...
- UC14 Verify Permission은 UC1, UC5, UC6, UC7, UC9, UC10, UC12, UC13, UC16에서 **include**된다. 또한 다이어그램에 선으로 모두 표시하지 않은 UC2, UC3, UC4, UC15 등 보호된 기능에서도 로그인, 프로젝트 소속, 역할 검사가 내부적으로 수행된다.
...
| Postconditions | - 시스템이 새 코멘트를 대상 이슈에 추가한다<br>- 시스템이 코멘트 작성자를 현재 사용자로, 작성 시간을 현재 시각으로 기록한다<br>- 시스템이 코멘트 추가 사실을 이슈 히스토리에 기록한다 |
...
2. 사용자가 코멘트를 입력한다
4. 시스템이 권한을 확인한다 (include UC14/권한 검사)
5. 시스템이 코멘트 본문이 비어있지 않은지 확인한다
6. 시스템이 코멘트를 저장하고 이슈 히스토리에 코멘트 추가 내역을 기록한다
7. 시스템이 호출 UC로 돌아간다

### Alternative Flows
- 4a. 사용자가 코멘트 추가 권한을 가지지 않는다
    - 4a1. 시스템이 권한 거부 메시지를 보여준다
    - 4a2. 흐름이 종료되고 호출 UC로 돌아간다
- 5a. 코멘트 내용이 비어있다
    - 5a1. UC1, UC4, UC5에서 호출된 경우 (extend)
        - 시스템이 코멘트 입력을 취소한 것으로 간주하여 호출 UC로 복귀한다
    - 5a2. UC6에서 호출된 경우 (include)
...
| Preconditions | - 사용자가 시스템에 로그인한 상태이다<br>- 대상 이슈가 시스템에 존재한다<br>- 대상 이슈는 DELETED 상태가 아니다<br>- 사용자가 대상 이슈가 속한 프로젝트의 active member이다 |
...
6. 시스템이 배정 또는 배정 변경 이력을 IssueHistory에 기록한다
...
- 3a. 필요한 역할에 대한 후보가 없다
    - 3a1. 시스템이 배정 가능한 후보가 없음을 보여준다
...
설계 메모: fixer와 resolver는 각각 ASSIGNED -> FIXED, FIXED -> RESOLVED 상태 전이에서 설정되며, 각 상태 전이는 IssueHistory(STATUS_CHANGED)에 기록된다.
...
8. 시스템이 상태 변경 사유 코멘트를 추가하고 IssueHistory(STATUS_CHANGED)를 기록한다
...
| Postconditions | - 조회 경로: 시스템이 해당 프로젝트의 DELETED 이슈 목록을 보여준다<br>- 삭제 경로: 시스템이 이슈의 status를 DELETED로 변경하고 deleted 전이 시각과 IssueHistory(STATUS_CHANGED)를 기록한다<br>- 삭제 경로: 시스템이 해당 이슈와 관련된 dependency를 제거한다<br>- 복구 경로: 시스템이 이슈의 status를 삭제 직전 상태(NEW 또는 CLOSED)로 변경하고 IssueHistory(STATUS_CHANGED)를 기록한다<br>- 복구 상태는 NEW/CLOSED -> DELETED 전이 때 기록된 IssueHistory(STATUS_CHANGED).previousValue에서 결정한다<br>- 복구 시 reporter, fixer, resolver, comments, history는 유지하고 assignee/verifier는 null 상태로 유지한다<br>- 삭제 시 제거된 dependency는 복구 시 자동 복원하지 않는다<br>- 복구된 이슈는 deleted 보관/FIFO 정리 후보에서 제외된다<br>- 영구 삭제 경로: 시스템이 선택한 DELETED 이슈와 관련 기록을 시스템에서 제거한다<br>- DELETED 상태 이슈가 보관 한도 30개를 초과하면 시스템이 FIFO 기준으로 가장 오래된 DELETED 이슈를 영구 삭제한다 |
| Trigger | PL이 Deleted Issue Management 화면을 열거나, NEW/CLOSED 이슈에서 삭제 액션을 선택하거나, DELETED 이슈 목록에서 복구/영구 삭제/보관 한도 정리 액션을 선택한다 |


### Brief Main Flow
1. PL이 Deleted Issue Management 화면을 열거나, 이슈 삭제/복구/영구 삭제/보관 한도 정리 액션을 선택한다
2. 시스템이 PL 권한과 대상 프로젝트 또는 대상 이슈의 현재 상태를 확인한다 (include UC14/권한 검사)
3. 조회 경로인 경우 시스템이 해당 프로젝트의 DELETED 이슈 목록을 보여준다
4. 삭제 경로인 경우 시스템이 이슈 상태를 NEW 또는 CLOSED에서 DELETED로 변경하고 관련 dependency를 제거한다
    - 삭제 후 DELETED 보관 한도를 초과하면 시스템이 오래된 DELETED 이슈부터 정리한다
5. 복구 경로인 경우 시스템이 이슈 상태를 DELETED에서 삭제 직전 상태(NEW 또는 CLOSED)로 복구한다
6. 영구 삭제 경로인 경우 시스템이 선택한 DELETED 이슈와 관련 기록을 시스템에서 제거한다
7. 보관 한도 정리 경로인 경우 시스템이 deleted 보관 한도 30개 초과 여부를 확인하고, 초과 시 가장 오래된 DELETED 이슈부터 FIFO로 영구 삭제한다
...
- *. PL이 삭제, 복구, 영구 삭제 또는 보관 한도 정리를 취소한다
...
| include | UC9 (삭제/복구 관리) | UC14 (권한 검사) | 삭제 이슈 조회, 삭제, 복구, 영구 삭제, 보관 한도 정리 시 시스템이 PL 권한과 대상 상태를 검사한다 |
...
| include | UC3 (이슈 검색/브라우즈) | UC14 (권한 검사) | 검색/브라우즈 시 시스템이 프로젝트 멤버 권한을 검사한다 |
| include | UC4 (이슈 상세 조회) | UC14 (권한 검사) | 상세 조회 시 시스템이 프로젝트 멤버 권한을 검사한다 |
...
| UC9 | Manage Deleted Issue | NEW/CLOSED 이슈 soft-delete, DELETED 이슈 조회/복구/단건 영구 삭제; 30개 초과 FIFO 영구 삭제는 soft-delete 후 시스템 내부 보관 정책으로 자동 수행 | Brief |
| UC10 | Statistics | 프로젝트 범위에서 일/월별 이슈 등록, 상태 변경, 코멘트, 상태/우선순위 분포 통계를 조회 | Brief |
