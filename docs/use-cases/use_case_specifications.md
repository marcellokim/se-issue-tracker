...
UC8, UC14, UC15, UC16은 관련 UC에서 include/extend 관계로 참조되며, 필요한 경우 Brief 수준으로만 설명한다.
...
6. 사용자가 화면을 확인한다
extension points: UC2(Add Comment), UC15(이슈 수정), UC16(이슈 우선순위 변경)
...
- 5c. PL이 우선순위 변경 액션을 선택한다
    - 5c1. UC4가 종료되고 UC16(이슈 우선순위 변경)이 시작된다
...
| extend | UC4 (이슈 상세 조회) | UC16 (이슈 우선순위 변경) | PL이 상세 화면에서 우선순위를 변경하고 싶을 때 추가한다 |
...
| UC16 | Change Priority | PL의 우선순위 변경 | Brief / extends UC4, includes UC14 |
...
- 2026-05-10 _#63 리뷰 반영 전면 보강: 작성 범위·핵심 UC 선정 근거·Actor 정의·include/extend 관계 설명 섹션 추가, UC4에 UC16(이슈 우선순위 변경) extension point 및 5c 대안 흐름 추가, UC6 REOPENED 자동 필드 설명 통일, 관계 요약표에 UC16->UC14 include 및 UC4->UC16 extend 추가, 전체 유스케이스 커버리지 매트릭스 추가, 데모 시나리오 매핑 추가_