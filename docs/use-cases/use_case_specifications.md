...
UC8, UC14, UC15는 관련 UC에서 include/extend 관계로 참조된다. UC8은 UC5에서 include되는 상태별 배정 후보 추천 subfunction이다. UC9와 UC16은 PL 전용 유스케이스로 UC14를 include하고, UC10은 Auth User 공통 조회 유스케이스로 둔다.
...
UC7-UC16은 전체 시스템 기능에는 포함되지만, 핵심 이슈 생명주기를 보조하거나 관리하는 기능이므로 전체 유스케이스 커버리지 매트릭스에서 기능 범위를 보인다. 이 중 UC9는 삭제/복구 회의 결정을 반영하기 위해 별도 Brief 명세를 추가한다.
...
| Tester | UC1 Register Issue, UC2 Add Comment, UC3 Search Issues, UC4 View Issue Detail, UC6 Change Issue State, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW 이슈) |
| Dev | UC1 Register Issue, UC2 Add Comment, UC3 Search Issues, UC4 View Issue Detail, UC6 Change Issue State, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW 이슈) |
| PL | UC1 Register Issue, UC2 Add Comment, UC3 Search Issues, UC4 View Issue Detail, UC5 Assign / Update Issue Assignment, UC6 Change Issue State, UC7 Manage Dependency, UC9 Manage Deleted Issue, UC10 Statistics, UC11 Log In, UC15 Edit Issue(자신이 reporter인 NEW 이슈), UC16 Change Priority |
...
`UC15 Edit Issue`는 `UC4 View Issue Detail`에서 reporter-only, NEW 상태 조건으로 진입하는 extension use case이며, UCD에서는 Auth User와 직접 연결하지 않고 UC4의 extend 관계로 표현한다.
...
| Postconditions | - 시스템이 새 이슈를 저장한다<br>- 시스템이 reporter를 현재 사용자로, reportedDate를 현재 시각으로, status를 NEW로 설정한다<br>- 사용자가 priority를 선택하지 않았다면 시스템이 MAJOR를 기본값으로 부여한다<br>- 시스템이 IssueHistory(CREATED)를 기록한다 |
...
| Postconditions | - 시스템이 새 코멘트를 이슈의 코멘트 history에 추가한다<br>- 시스템이 코멘트 등록자를 현재 사용자로, 코멘트 작성 시간을 현재 시각으로 기록한다<br>- 시스템이 IssueHistory(COMMENTED)를 기록한다 |
| Trigger | - UC1(이슈 등록), UC4(이슈 상세 조회), UC5(이슈 배정 및 배정 변경)의 extension point에서 사용자가 코멘트 추가를 선택 (extend)<br>- UC6(이슈 상태 변경)의 코멘트 입력 단계에서 자동 시작 (include) |
...
3. 사용자가 title keyword, assignee, reporter, status, priority, reportedDate 범위 등의 검색 조건을 입력한다.
4. 사용자가 검색 버튼을 누른다
...
- 5a. PL이 이슈 배정 또는 배정 변경 액션을 선택한다
    - 5a1. UC4가 종료되고 UC5(이슈 배정 및 배정 변경)가 시작된다
...
|---|---|
| ID | UC5 |
| 이름 | Assign / Update Issue Assignment |
| 범위 | Issue Tracking System |
| 수준 | User goal |
| Primary Actor | PL |
...
| Preconditions | - PL이 시스템에 로그인한 상태이다<br>- 대상 이슈가 시스템에 존재한다<br>- 대상 이슈의 상태가 NEW, REOPENED, ASSIGNED 또는 FIXED이다 |
| Postconditions | - NEW: 시스템이 이슈의 assignee를 PL이 선택한 Dev로, verifier를 PL이 선택한 Tester로 설정하고 status를 ASSIGNED로 변경한다<br>- REOPENED: 시스템이 assignee와 verifier를 PL이 선택한 값으로 재설정하고 status를 ASSIGNED로 변경한다. 기존 fixer/resolver 기록은 보존한다<br>- ASSIGNED: 시스템이 assignee를 PL이 선택한 Dev로 변경하고 status를 ASSIGNED로 유지한다<br>- FIXED: 시스템이 verifier를 PL이 선택한 Tester로 변경하고 status를 FIXED로 유지한다<br>- 배정 또는 배정 변경이 완료되면 시스템이 IssueHistory(ASSIGNMENT_CHANGED)를 기록한다<br>- 단, NEW -> ASSIGNED 또는 REOPENED -> ASSIGNED처럼 status가 함께 변경되는 경우에는 IssueHistory(STATUS_CHANGED)도 함께 기록한다 |
| Trigger | PL이 이슈 상세 화면에서 이슈 배정 또는 배정 변경 액션을 선택한다 |

### Main Flow
1. PL이 이슈 배정 또는 배정 변경 버튼을 누른다
2. 시스템이 권한과 전이 규칙을 확인한다 (include UC14/권한 검사)
3. 시스템이 UC8 Recommend Assignment Candidates를 include하여 대상 이슈 상태에 맞는 배정/변경 옵션과 추천 후보를 보여준다
    - NEW(UC5-N): Dev assignee 후보와 Tester verifier 후보를 보여준다. 이때 대상 이슈의 reporter가 Tester인 경우, 시스템은 해당 reporter를 기본 verifier 후보로 우선 제시할 수 있다.
    - REOPENED(UC5-R): Dev assignee 후보와 Tester verifier 후보를 보여준다
    - ASSIGNED(UC5-RA): Dev assignee 후보와 현재 assignee를 보여주며 testerList/currentTester는 반환하지 않는다
    - FIXED(UC5-RV): Tester verifier 후보와 현재 verifier를 보여주며 assignee 후보는 반환하지 않는다
4. 시스템이 과거 resolved/closed 이슈의 fixer/resolver 이력을 우선 참고하여 후보를 추천한다.(include UC8/fixer/resolver 이력 기반 assignee/verifier 후보 추천)
5. PL이 상태별로 필요한 assignee 또는 verifier를 선택한다
extension point: UC2(코멘트 추가)
6. 시스템이 상태별로 assignee, verifier, status를 갱신한다
7. 시스템이 배정 또는 배정 변경 이력을 IssueHistory에 기록한다
...
- 3a. Dev 후보가 필요한 상태(NEW, REOPENED, ASSIGNED)에서 배정 가능한 Dev가 없다
    - 3a1. 시스템이 배정 가능한 Dev가 없다는 메시지를 보여준다
    - 3a2. PL이 이전 화면으로 돌아간다
- 3b. Tester 후보가 필요한 상태(NEW, REOPENED, FIXED)에서 배정 가능한 Tester가 없다
    - 3b1. 시스템이 배정 가능한 Tester가 없다는 메시지를 보여준다
    - 3b2. PL이 이전 화면으로 돌아간다
- 3c. ASSIGNED 상태에서 assignee만 재배정한다 (UC5-RA)
    - 3c1. 시스템은 testerList/currentTester를 반환하지 않고 Dev 재배정 옵션만 보여준다
    - 3c2. 기존 fixer 값이 존재하는 경우 이를 유지한 채 assignee만 변경한다
- 3d. FIXED 상태에서 verifier만 교체한다 (UC5-RV)
    - 3d1. 시스템은 Tester verifier 후보와 교체 옵션을 보여준다
    - 3d2. 시스템은 status=FIXED와 fixer를 유지한 채 verifier만 변경한다
- *. PL이 배정을 취소한다
...
| Postconditions | - 시스템이 이슈의 status를 목표 상태로 변경한다<br>- 시스템이 전이에 따른 자동 필드를 갱신한다<br>- 시스템이 상태 변경 사유 코멘트를 코멘트 history에 추가한다<br>- 시스템이 IssueHistory(STATUS_CHANGED)를 기록한다 |
...
| FIXED | RESOLVED | Tester (verifier 본인) | resolver = 현재 Tester, 기존 assignee/verifier/fixer 유지 | verifier가 수정 결과를 검증한다. 단, blocking issue가 모두 RESOLVED 또는 CLOSED일 때만 허용된다 |
| FIXED | ASSIGNED | Tester (verifier 본인) | 기존 assignee, verifier, fixer 유지 | verifier가 수정이 불충분하다고 판단하여 상태를 되돌림 |
| RESOLVED | CLOSED | PL | assignee=null, verifier=null | PL이 검증 완료된 이슈를 종료시킴. fixer와 resolver는 보존한다 |
| CLOSED / RESOLVED | REOPENED | PL | fixer/resolver 보존 | PL이 종료된 이슈를 재개. assignee/verifier는 자동 복원하지 않고, PL은 필요 시 UC5를 통해 배정한다 |

설계 메모: fixer와 resolver는 각각 ASSIGNED -> FIXED, FIXED -> RESOLVED 상태 전이에서 설정되며, 동일한 상태 전이는 IssueHistory(STATUS_CHANGED)에 함께 기록된다.
...
7. 시스템이 이슈의 status를 갱신하고, 전이에 따른 자동 필드를 처리한다
    ...
    - FIXED -> RESOLVED: dependency guard를 통과하면 resolver를 현재 Tester로 기록하고 기존 assignee, verifier, fixer를 유지한 채 상태를 변경한다
    - FIXED -> ASSIGNED: 기존 assignee, verifier, fixer를 유지한다. 이후 다시 FIXED 전이가 발생하면 fixer는 그 시점의 Dev로 갱신될 수 있다
    - RESOLVED -> CLOSED: assignee와 verifier를 null로 제거하고 fixer와 resolver를 보존한 채 상태를 최종 종료한다
    - CLOSED/RESOLVED -> REOPENED: fixer와 resolver를 보존한 채 재작업 가능 상태로 설정한다. assignee/verifier는 자동 복원하지 않으며 PL은 필요 시 UC5를 통해 배정한다
8. 시스템이 상태 변경 이력을 IssueHistory(STATUS_CHANGED)에 기록한다
...
| Preconditions | - PL이 시스템에 로그인한 상태이다<br>- 대상 이슈가 시스템에 존재한다<br>- 삭제 경로: 대상 이슈의 상태가 NEW 또는 CLOSED이다<br>- 복구 경로: 대상 이슈의 상태가 DELETED이고 아직 영구 삭제되지 않았다 |
| Postconditions | - 삭제 경로: 시스템이 이슈의 status를 DELETED로 변경하고 deleted 전이 시각과 IssueHistory(STATUS_CHANGED)를 기록한다<br>- 복구 경로: 시스템이 이슈의 status를 삭제 직전 상태(NEW 또는 CLOSED)로 변경하고 IssueHistory(STATUS_CHANGED)를 기록한다<br>- 복구 상태는 NEW/CLOSED -> DELETED 전이 때 기록된 IssueHistory(STATUS_CHANGED).previousValue에서 결정한다<br>- 복구 시 reporter, fixer, resolver, comments, history는 유지하고 assignee/verifier는 null 상태로 유지한다<br>- 삭제 시 제거된 dependency는 복구 시 자동 복원하지 않는다<br>- 복구된 이슈는 deleted 보관/FIFO 정리 후보에서 제외된다<br>- DELETED 상태 이슈가 보관 한도 30개를 초과하면 시스템이 IssueHistory(STATUS_CHANGED, newValue=DELETED).changedDate 기준 FIFO로 가장 오래된 DELETED 이슈를 영구 삭제한다 |
| Trigger | PL이 NEW/CLOSED 이슈에서 삭제 액션을 선택하거나, DELETED 이슈 목록에서 복구 액션을 선택한다 |

### Brief Main Flow
1. PL이 NEW/CLOSED 이슈의 삭제 또는 DELETED 이슈의 복구 액션을 선택한다
2. 시스템이 PL 권한과 대상 이슈의 현재 상태를 확인한다 (include UC14/권한 검사)
3. 삭제 경로인 경우 시스템이 이슈 상태를 NEW 또는 CLOSED에서 DELETED로 변경하고 관련 dependency를 제거한다
4. 복구 경로인 경우 시스템이 이슈 상태를 DELETED에서 삭제 직전 상태(NEW 또는 CLOSED)로 복구한다
...
- deleted 전이 시각과 복구 대상 상태는 별도 Issue 속성이 아니라 IssueHistory의 STATUS_CHANGED 기록에서 결정한다.
...
| include | UC5 (이슈 배정 및 배정 변경) | UC8 (Assignment Candidate 추천) | PL의 배정/배정 변경 시 시스템이 Issue status에 따라 assignee/verifier 후보를 추천한다 |
| include | UC5 (이슈 배정 및 배정 변경) | UC14 (권한 검사) | 배정/배정 변경 시 시스템이 권한과 전이 규칙을 검사한다 |
...
| extend | UC5 (이슈 배정 및 배정 변경) | UC2 (코멘트 추가) | PL이 이슈를 배정하거나 변경할 때 사유를 코멘트로 남기고 싶을 때 추가한다 |
...
| UC8 | Recommend Assignment Candidates | Issue status와 fixer/resolver 이력 기반 assignee/verifier 후보 추천 | Brief / included by UC5 |
| UC9 | Manage Deleted Issue | NEW/CLOSED 이슈 soft-delete 및 DELETED 이슈 복구; 30개 초과 FIFO 영구 삭제는 내부 보관 정책 | Brief |
| UC10 | Statistics | 일/월별 이슈 발생 및 트렌드 분석(Auth User가 기간, 상태, 담당자, 프로젝트 범위(filters.scope)에 따라 일/월별 이슈 발생 및 처리 추세를 조회하는 기능) | Brief |
...
- 2026-05-11 _대면회의 반영: UC9를 Manage Deleted Issue로 수정, 삭제/복구 Brief 명세와 FIFO 영구 삭제 내부 보관 정책을 추가_
- 2026-05-13 _SSD/도메인 모델 정합성 반영: UC9 삭제 경로를 NEW/CLOSED→DELETED로 확장, 복구 경로를 DELETED→삭제 직전 상태로 수정, deleted 전이 시각과 복구 상태를 IssueHistory 기반으로 결정하도록 명시_
- 2026-05-14 _팀 회의 반영: UC5를 배정/배정 변경 범위로 확장하고 UC8을 Recommend Assignment Candidates로 재정의, 상태별 assignee/verifier 후보 반환 정책과 ASSIGNMENT_CHANGED 이력, UC6 STATUS_CHANGED 이력, fixer/resolver-history 동기화 메모, UC10 filters.scope, resolver 보존 정책을 반영_
