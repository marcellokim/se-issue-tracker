...
Non-admin 사용자가 프로젝트를 선택하면 이슈 목록 화면은 프로젝트 내부 화면 역할을 한다. 따라서 화면 진입 시 `ProjectController.viewProjectNonAdminDetail(projectId)`로 프로젝트 기본 정보를 조회하고, `IssueController.viewProjectIssues(projectId)`로 프로젝트의 삭제되지 않은 이슈 목록을 조회한다.
...
의존성 영역은 이슈 상세에서 현재 이슈를 막는 목록과 현재 이슈가 막고 있는 목록을 나누어 보여준다.
...
- `purgeOverflow`는 `deleteIssue` 시 자동 실행되므로 UI에 별도 버튼 없음
...
로그인
  ...
  │                                         └──→ 멤버 추가/제거
  │
  └── PL|Dev|Tester ──→ 프로젝트 목록 ──→ 이슈 목록(프로젝트 기본정보 + 프로젝트 이슈) ──→ 이슈 상세
                                            │               │
