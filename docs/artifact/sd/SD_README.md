...
- Service와 Repository port는 설계상 존재하지만, SD에서는 핵심 책임이 아닌 경우 note나 guard로 축약한다.
...
6. 확정 정책 기준: operation 이름, 주요 상태 전이 방향, 필수 정책 확인
...
- Pure Fabrication / Indirection: Service, repository, policy, clock은 설계상 필요한 협력자이지만 제출용 SD에서는 핵심 책임이 아닐 때 축약한다.
...
- Service/Repository/JDBC 호출 순서를 그대로 복사하지 않는다.
- 확정된 정책 방향이 어긋나지 않도록 operation 이름, 상태 전이, 필수 comment, dependency guard, deleted issue 정책은 반영한다. dependency는 resolve 시점의 guard로 표현하며, resolve 성공만으로 자동 제거하지 않는다.
- 삭제/복구처럼 repository port가 많은 일을 처리하는 흐름이라도, SD 작성 단계에서는 soft delete/restore의 개념적 사후조건을 `Issue`, `IssueHistory`, `IssueDependency` 협력으로 표현할 수 있다.
...
- PUML: `sd_puml/`
- GRASP 설명: `sd_grasp/`
- PNG/SVG 이미지는 필요할 때 PlantUML로 랜더링한다. 저장소에서는 `.puml`과 GRASP 설명 파일을 원본으로 관리한다.
