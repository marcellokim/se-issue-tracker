...
현재 실행 조립의 중심은 `ApplicationBootstrap`과 `JdbcRepositoryFactory`이다.

### `ApplicationContext` / `ApplicationBootstrap`

`ApplicationBootstrap`는 Oracle 환경에서 실제 runtime collaborator를 조립하는 composition root이다.
...
`ApplicationContext`는 JavaFX 실행에 필요한 `AuthenticationService`와 `DemoDashboardPresenter`를 노출하는 얇은 UI context로 유지한다.

즉, application/service/controller/UI/CLI가 직접 JDBC 구현체를 생성하지 않고, 실행 시작 지점에서 조립한다.

### CLI Boundary

`CommandLineEntryPoint`는 기존 `--cli-demo`와 `--login-check` mode를 선택하고, 각 command object로 위임한다.

CLI command는 service 결과를 출력 형식으로 변환한다. CLI command가 repository interface나 JDBC 구현체를 직접 import하지 않도록 `ArchitectureBoundaryTest`가 `cli` package의 repository/persistence import를 막는다.

이 구조는 새 CLI 기능을 추가한 것이 아니라, 기존 CLI smoke/demo 흐름의 presentation 책임을 `Main`에서 분리한 리팩토링이다.
...
`JdbcIssueRepository`는 `IssueRepository` contract를 유지하되 내부 책임을 package-private helper로 나눈다.
...
이 분리는 SQL 결과, 정렬, deleted issue 포함/제외 기준, rollback, generated key handling, comment/history persistence order를 바꾸지 않는 pure refactor이다.
...
현재 확인된 boundary guard 기준에는 임시 예외가 없다. 예외는 영구 설계가 아니라 debt marker로만 허용하며, 새 예외를 추가해야 할 때는 해당 clean-code slice에서 제거 계획을 같이 남긴다.
...
- 리팩토링 PR의 문서 변경은 구조와 책임 배치 설명에 한정한다. 별도 feature PR과 테스트 근거가 없는 기능은 traceability나 README에서 완료로 표시하지 않는다.
