...
| 개발 보조 | UC12 계정 관리 | Update Account | Admin | `updateAccount(userId, name, role)` | `User.name`, `User.role` 변경 | `isActive`는 Deactivate Account에서만 처리 |
...
| 내부 공통 | UC14 권한 검사 | Verify Permission | Actor/Internal | `verifyPermission(actorId, operation, resourceId)` | 권한 있음/없음 alt로 base operation 진행 또는 거부 | 직접 사용자 목표 SSD가 아니라 include되는 공통 권한 검사 reference |
| 개발 핵심 | UC15 이슈 수정 | Edit Issue | Reporter | `editIssue(issueId, newTitle, newDescription)` | status `NEW`인 본인 이슈 title/description 수정, history 저장 | priority는 UC16, status는 UC6 |
...
10. `editIssue(issueId, newTitle, newDescription)`
