...
| 위치 | 역할 |
|---|---|
| `uc/` | Use Case Diagram과 주요 Use Case 명세 |
...
| `oc/` | SSD/UC에 대응되는 Operation Contract |
| `ssd/ssd-candidate-catalog.md` | 보고서 제출용 SSD 후보와 선정 이유 |
...
UML 문서는 현재 코드의 정책 방향을 반영하되, 코드 호출 순서를 그대로 옮기는 문서는 아니다. 각 문서는 다음 수준으로 읽는다.
...
- `oc/`는 SSD/UC의 system operation에 대응되는 사후조건을 정리한다. Postcondition은 완료된 결과 기준으로 작성한다.
...
```sh
plantuml -tsvg docs/artifact/uc/*.puml docs/artifact/domain/*.puml docs/artifact/dcd/*.puml docs/artifact/architecture/*.puml docs/artifact/ssd/ssd-*.puml docs/artifact/sd/sd_puml/*.puml
```
...
- `docs/artifact/_shared/its-uml-style.puml`, `docs/artifact/ssd/ssd-style.puml`, `docs/artifact/sd/_shared/its-uml-style.puml`은 include 전용 파일이다.
- `ssd/SSD_README.md`, `sd/SD_README.md`, `sd/sd_grasp/*.txt`, `oc/*.md`는 PlantUML 렌더 대상이 아니다.
