...
| UC3 Search / Browse Issues | 프로젝트별 일반 이슈를 reporter/assignee/verifier/status/priority/date 조건으로 조회하고 DELETED 이슈는 제외함 | `IssueController.searchIssues`, `viewRelatedProjectIssues` | `IssueServiceTest`, `IssueControllerTest`, `IssueListPresenterTest` | JavaFX `07-issue-list`, Swing issue list; PR 230, PR 255 | 완료 |
...
| UC6 Change Issue State | 역할과 상태 조건에 맞게 ASSIGNED/FIXED/RESOLVED/CLOSED/REOPENED 전이를 수행하고 사유 코멘트를 기록함 | `IssueStateController.changeStatus` | `IssueStateServiceTest`, `IssueStateControllerTest`, `IssueWorkflowTest` | JavaFX `09-status-change`, Swing status dialog; PR 89, PR 233 | 완료 |
| UC7 Manage Dependency | PL이 의존성을 추가/제거하고 순환, 중복, unresolved blocking issue를 검사함 | `IssueController.addDependency`, `removeDependency`, `viewProjectDependencies` | `IssueDependencyTest`, `IssueServiceTest`, `IssueDependency*Test` | JavaFX `12-dependency`, dependency graph; Swing dependency action; PR 236, PR 244, PR 251 | 완료 |
