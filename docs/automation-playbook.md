...
| `.githooks/pre-commit` | 구현 | 위험한 브랜치 작업/설정 누락 방지 | 초기 설정 시 자동 설치 |
...
정렬한다. 이 워크플로우는 보정 자동화이므로 브랜치 보호 필수 체크에는 넣지 않는다.

공개 GitHub 이력에는 팀원 이름과 저장소 계정만 남긴다. 외부 도구명, 기계적 작성 표기, 공동작성자
...
| 프로젝트 자동 추가 워크플로우 | 부분 구현 | 이슈/PR를 GitHub 프로젝트에 자동 추가 |
| 프로젝트 정합성 유지 워크플로우 | 구현 | 이슈/PR 이벤트와 매일 00:17 KST에 프로젝트 상태 정렬/점검 |
| 워크플로우 보호 | 구현 | 일반 팀원의 `main` PR, 잘못된 head 브랜치, 보호 자동화 수정 시도 차단 |
| 리뷰 봇 정책 | 구현 | 폐기된 외부 리뷰 앱 설정은 제거하고, 저장소에 등록된 코드 리뷰 앱 중심으로 한국어·저소음 리뷰를 유지 |
| CODEOWNERS | 기본 구현 | 리뷰 책임자 구조 준비 |
| GitHub 초기 설정 스크립트 | 구현 | 라벨/마일스톤/프로젝트/저장소 변수/저장소 설정/브랜치 보호 규칙 정렬 |

### 2-4. 보안 자동화

| 항목 | 상태 | 비고 |
| --- | --- | --- |
| Dependabot 보안 업데이트 | 활성화 | 의존성 취약점 알림/PR. 자동 업데이트 PR은 `dev` 대상으로 생성 |
| Secret scanning | 활성화 | 민감정보 탐지 |
| Secret scanning push protection | 활성화 | push 시점 차단 |
| Private vulnerability reporting | 활성화 | 비공개 보안 신고 |
| 보안 코드 분석 워크플로우 | 활성화 | 한국어 체크 이름으로 Java/Kotlin, Python, GitHub Actions 분석 |
...
### 이미 켜진 것
- Dependabot 보안 업데이트 (`dev` 대상 PR 생성)
- Secret scanning
...
### 현재 비활성 상태인 것
- 코드 리뷰 GitHub App 설치/활성화: 저장소 설정 파일은 준비되어 있지만, GitHub App 설치와 권한 승인은 관리자가 직접 확인해야 함
- Secret scanning non-provider patterns
- Secret scanning validity checks

위 항목은 GitHub App 설치, 지원 플랜, 저장소 상태에 따라 바로 활성화되지 않을 수 있습니다.

---

## 4. GitHub 초기 설정 스크립트가 실제로 해주는 일
...
- `main`/`dev` 브랜치 보호 규칙 동기화
- `빌드와 테스트`/`워크플로우 정책 검사` 필수 체크 설정
- GraphQL 한도 영향을 받는 프로젝트/PR 메타데이터 보정 자동화는 필수 체크에서 제외
- PR 리뷰 1개, 최신 기준선, force push/delete 금지 설정

### 자동으로 맞추지 않는 항목
- CODEOWNERS 실제 사용자 ID 반영
- `ADD_TO_PROJECT_PAT` secret
- GitHub 프로젝트 보드 보기 세부 구성

즉, 초기 설정 스크립트는 반복되는 저장소 정책을 맞추지만, 팀원 권한과 프로젝트 화면 구성처럼 계정/수업 운영에 묶인 항목은 관리자가 직접 확인합니다.
...
## 8. 자동화가 대체하지 않는 수동 확인 항목
아래 항목은 자동화가 보조하지만, 최종 판단은 사람이 확인한다. 최종 제출 기준으로는 검증과 제출물 업로드 확인까지 완료했다.

1. **Project/board 운영 설정**: `ADD_TO_PROJECT_PAT` secret은 Project 자동 추가/정렬 보조 기능이며, 최종 증빙은 Project 화면과 issue/PR/milestone 상태를 사람이 확인한다.
2. **코드 리뷰 GitHub App 설치/권한 확인**: 관련 저장소 설정은 준비되어 있으며, App 설치 자체는 관리자가 GitHub UI에서 확인한다.
3. **CODEOWNERS 실제 팀원 ID 반영**: 제출 증빙에는 GitHub issue/PR/commit 이력을 우선 사용한다.
4. **GitHub 프로젝트 Board/Table 최종 뷰 정리**: 최종 제출 전 Project, milestone, issue, PR, CI 상태를 확인했다.
5. **최종 검증과 패키징**: `./gradlew check`, `verifySubmissionMetadata`, CI, source package 기준 검증을 완료했다.
6. **문서/발표/Q&A 산출물의 사람 검토**: 최종 PDF, slides, demo video, README/source package 제출 플랫폼 업로드 확인을 완료했다.

---

## 9. 추천 운영 흐름
1. 초기 설정 실행
2. 저장소 관리자만 GitHub 초기 설정 실행
3. 이슈 생성
...
## 10. 신규 작업 시작 전에 확인할 것
- [ ] 팀원 모두 Java 21 / Git / gh 준비
- [ ] GitHub 프로젝트 접근 가능
- [ ] `dev`에서 브랜치 생성 흐름 정상 작동
- [ ] PR 시 `빌드와 테스트` 체크가 실제로 보임
- [ ] 이슈 템플릿 / PR 템플릿이 기본 브랜치 기준으로 최신 상태
...
- 리뷰 정책이 바뀔 때
- 제출 형식이 바뀔 때
- GitHub 프로젝트 운영 방식이 바뀔 때

### 자동화를 점검해야 하는 순간
- CI가 자주 깨질 때
- 팀원이 새로 합류할 때
- 프로젝트 상태와 이슈 라벨이 어긋날 때
