...
| 리뷰 봇 정책 | 구현 | 폐기된 외부 리뷰 앱 설정은 제거하고, ChatGPT Codex Connector와 Gemini Code Assist 중심으로 한국어·저소음 리뷰를 유지 |
...
### 현재 비활성 상태인 것
- Gemini Code Assist GitHub App 설치/활성화: 저장소 설정 파일은 준비되어 있지만, GitHub App 설치와 권한 승인은 관리자가 직접 확인해야 함
...
1. **`ADD_TO_PROJECT_PAT` secret 추가 여부 결정**: 없으면 프로젝트 자동 추가/정렬 워크플로우가 건너뜁니다.
2. **Gemini Code Assist GitHub App 설치/권한 확인**: `.gemini/config.yaml`과 `.gemini/styleguide.md`는 repo 설정이며, App 설치 자체는 관리자가 GitHub UI에서 확인합니다.
3. **CODEOWNERS 실제 팀원 ID 반영**
4. **GitHub 프로젝트 Board/Table 최종 뷰 정리**
5. **최종 제출 직전 `./gradlew check` + 패키징 재실행**
6. **문서/발표/Q&A 산출물의 사람 검토**
