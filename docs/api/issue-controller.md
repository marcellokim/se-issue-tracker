...
`viewProjectIssues`는 특정 프로젝트의 전체 이슈를 반환한다. 프로젝트 멤버(PL, DEV, TESTER)는 역할에 관계없이 해당 프로젝트의 모든 이슈를 조회할 수 있다. 검색 기능을 통해 assignee, reporter, verifier 등 조건별 필터링이 가능하다.
