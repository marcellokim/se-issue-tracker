...
`viewProjectIssues`는 특정 프로젝트 내부의 일반 이슈 목록을 반환한다. PL/DEV/TESTER는 자신이 active member로 참여한 프로젝트의 `DELETED`가 아닌 이슈 전체를 볼 수 있고, 원하는 이슈는 `searchIssues`의 keyword, status, priority, reporterId, assigneeId, verifierId, reportedDate 범위 필터로 좁혀 찾는다.
...
| `implementation-extra` | 검색, 상세 조회, action 조회, 댓글 수정/삭제, 프로젝트 이슈 목록 조회 API는 필수 OC 목록 밖에 추가로 구현되어 있다. |
