...
| `renameAccount`, `changeAccountRole` | UC12, SSD-17; supporting API, not part of required OC list | `User` role/name lifecycle in DCD; implementation `User.rename`, `User.changeRole`, `AccountService.rejectRoleChangeWithProjectResponsibility` |
...
- Throws `NullPointerException("role must not be null")` when account creation or role-change receives a null `role`.
