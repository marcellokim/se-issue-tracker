...
## 3. 과제 핵심 요구사항 요약
과제 요구사항 원문은 저작권과 공개 범위를 고려해 현재 소스 트리와 제출용 source zip에 포함하지 않고 팀 로컬/제출 자료로 관리합니다. 이후 커밋에서도 원문 PDF를 재추가하지 않으며, 저장소에는 요구사항 요약, 추적표, 설계/구현 근거만 남깁니다. 아래 요약이나 추적 문서가 애매할 때는 팀이 보유한 과제 원문을 우선합니다.
...
- [x] Swing 전체 UI 구현
- [x] 통계 / 추천 기능 구현
- [x] 최종 발표 자료 / 동영상 / 프로젝트 문서 PDF 완성
- [x] 최종 제출 패키지 freeze 및 증빙 정리
...
세부 환경변수, 앱 DB 초기화, CI 자동 실행 계약은 [docs/execution-test-guide.md](docs/execution-test-guide.md)를 확인합니다.
...
### 애플리케이션 실행 확인
기본 실행은 JavaFX UI이며, Swing UI는 별도 Gradle task로 실행합니다. Oracle DB를 사용하는 데모 실행은 [docs/execution-test-guide.md](docs/execution-test-guide.md)의 앱 DB 준비 절차를 먼저 따릅니다.
...
PR 제목은 브랜치 타입을 기준으로 `type: 한국어 요약` 형식을 사용합니다. 예를 들어 `docs/160-api-review` 브랜치는 `docs: 전체 API 명세 검토 및 정리`처럼 작성합니다. scope가 필요하면 `docs(api): ...`처럼 붙입니다. 이 형식은 팀 작성 기준이며 별도 자동화 실패 조건은 아닙니다.
...
- **제출 zip 스크립트**: 소스 zip과 `README.txt` 생성
...
├─ docs/                    # 과제 문서, 사용 가이드, UML/PlantUML 원본
...
발표 슬라이드, 프로젝트 문서 PDF, 프로젝트 소개 동영상은 저장소 소스 패키지와 별도 산출물로 준비하여 제출 플랫폼에 업로드 완료했습니다.
...
- 코드 수정이나 리뷰 전에는 `docs/README.md`를 시작점으로 UCD, domain model, logical architecture, SSD, SD, DCD, Operation Contract를 확인하고 현재 구현 구조를 파악합니다. 설계 판단은 다이어그램, 구현 세부 문서, 실제 호출 경로를 함께 대조합니다.
- 공개 GitHub 이력에는 팀원 이름과 저장소 계정만 남깁니다. 외부 도구명, 기계적 작성 표기, 공동작성자 trailer는 commit hook과 저장소 감사에서 차단합니다.
...
- 로컬 QA 이미지 산출물, 강의 원문 참고 자료, 개인 지침/메모 파일, 보조 리뷰 도구 설정은 제출 zip에 포함하지 않음
- 문서와 코드 이해 여부를 팀 전체가 직접 검토
