...
> 이 문서는 이슈 관리 시스템의 include + extend 관계를 포함한 유스케이스 6개에 대한 Fully Dressed 명세입니다.
...
5. 시스템이 입력값을 확인한다
6. 시스템이 이슈를 저장한다
7. 시스템이 등록 완료 화면을 보여준다
extension point : UC2(Add comment)

### Extensions (대안 흐름)
- 5a. Title 또는 Description이 비어있다
    - 5a1. 시스템이 누락 항목을 표시한다
    - 5a2. Tester가 다시 입력한다 (단계 3으로 복귀)
- 7a. Tester가 등록 직후 코멘트 작성을 선택한다
    - 7a1. UC2 흐름이 시작된다
    - 7a2. 코멘트 작성이 끝나면 흐름이 종료된다
- *. Tester가 등록을 취소한다
...
| 범위 | Issue Tracking System |
| 수준 | Subfunction |
| Primary Actor | Authenticated User : UC1, UC4에서 호출 시<br>없음 : UC5에서 호출 시 |
...
| Trigger | - UC1 또는 UC4에서 호출 (<<extend>>) <br> - UC5에서 호출 (<<include>>) |
...
- 6a. 사용자가 이슈 상태 변경 액션을 선택한다
    - 6a1. 선택된 이슈의 상태가 변경된다 
    - 6a2. 이슈 상태 변경 내역이 시스템에 반영된다
...
4. include UC2(Add Comment)
5. 시스템이 전이에 따른 자동 필드를 채운다
    - NEW -> ASSIGNED: assignee를 PL이 선택한 dev로 채운다
    - ASSIGNED -> FIXED: fixer를 현재 Dev로 채운다
    - FIXED -> RESOLVED: 자동으로 채우는 필드는 없다
6. 시스템이 변경 사항을 저장한다
7. 시스템이 변경 이력을 시스템 메시지로 코멘트 history에 남긴다
8. 시스템이 갱신된 상세 화면을 보여준다

### Extensions
- 3a. 사용자가 PL이고 Assignee 추천 기능을 사용한다
    - extension point : UC6(Recommend Assignee)
- 5a. 사용자가 사유 코멘트를 비워두고 저장한다
    - 5a1. UC2의 대안 흐름 4a-1로 진입하여 코멘트 입력을 강제한다
    - 5a2. 코멘트가 입력될 때까지 단계 6으로 진행하지 못한다
- 6a. 사용자의 역할이 요청한 전이를 수행할 수 없다
    - 6a1. 시스템이 거부 사유와 가능한 전이 목록을 보여준다
...
| Preconditions | - UC5가 진행 중이고, PL이 NEW 이슈에 assignee를 지정하려는 시점이다<br>- 대상 프로젝트에 RESOLVED 또는 CLOSED 상태의 과거 이슈가 1건 이상 있다 |
| Postconditions  | 시스템이 추천 후보 dev를 최대 3명까지 적합도 순서로 PL에게 보여준다 |
| Trigger | UC5의 단계 4에서 PL이 NEW 이슈를 배정하려는 조건이 충족된다 |
| Extension Point | UC5의 단계 3 (assignee 입력 시점) |
